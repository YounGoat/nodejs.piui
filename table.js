...
const MODULE_REQUIRE = 1
	...
	, uncolors = require('jinang/uncolors')
...
function table(rows, options) {
	...
	if (options.border == 'thin') {
		let printBorder = (I, row, isHead) => {
			let parts = [];
			let pushBorder = (x, y, repeat = 1) => {
				parts.push(colors.dim(BORDER.thin[x][y].repeat(repeat)));
			};
			pushBorder(I, 0);
			columns.forEach((column, index) => {
				if (index > 0) parts.push(pushBorder(I, 2));
				if (row) {
					if (isHead) {
						column = Object.assign({}, column, { align: 'center' });
					}
					parts.push(formatCell(column, row[column.name]));
				}
				else {
					pushBorder(I, 1, column.size + options.padding * 2);
				}
			})
			pushBorder(I, 3);
